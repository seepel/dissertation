...
OTHER_SRC_FILES	= \
		...
		simulation.tex \
		analysis.tex \
		triggerselection.tex \
		particleselection.tex \
		eventselection.tex \
		pzetadiff.tex \
		zmumuveto.tex \
		background.tex \
		prominantbackgrounds.tex \
		controlregions.tex \
		backgroundestimation.tex \
		systematics.tex \
		montecarlocorrections.tex \
		systematicuncertainties.tex \
		results.tex \
		selectedevents.tex \
		limits.tex \
		dissertation.bib \
		...
		if test "$(CHAP_CITE)" ; then \
			for file in bogus $(CHAP_CITE) ; do \
				if test "$$file" != "bogus"; then \
					...
					echo "NOT RUNNING BIBTEX ON FILE: $$file"; \
				fi ; \
			done ; \
		else \
